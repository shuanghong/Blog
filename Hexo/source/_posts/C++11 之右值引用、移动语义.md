...
## 右值引用、右值
为了实现移动语义, C++11 引入了右值引用, 即绑定到右值的引用, 采用 T&& 语法形式. 传统的引用被称为左值引用, 采用 T& 语法形式.
...
1. STL 容器的相关函数如 vector::push_back, 会对参数对象进行复制, 这就会造成对象内存的额外创建. 原意是想把参数 push_back 进去就行了. 如下代码:
